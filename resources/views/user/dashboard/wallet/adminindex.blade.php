...
@section('user-contant')
    <style>
        ...
        .text-end {
            text-align: right;
            width: 120px;
            /* Optional: You can set a width for the numbers to align them better */
        }
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span id="modalCourierCharge" class="text-end"></span>
                                            </div>
                                            <div class="d-flex justify-content-between">
                                                <strong>Tax Amount:</strong>
                                                <span id="modalTaxAmount" class="text-end"></span>
                                            </div>
                                            <div class="d-flex justify-content-between">
                                                <strong>COD Charge:</strong>
                                                <span id="modalCodCharge" class="text-end"></span>
                                            </div>
                                            <hr>
                                            <div class="d-flex justify-content-between">
                                                <strong>Total Amount:</strong>
                                                <span id="modalAmount" class="text-end"></span>
                                            </div>
                                        </div>
                            ...
                            <div class="card-header-form">
                                <div class=" text-right " style="margin-right:2% ;">
                                    <a href="javascript:void()"
                                        onclick="window.location.href='{{ route('admin.add-balance1') }}'"
                                    ...
                                    <div class="card-body" style="background-color: #bfbfbf;">
                                        <form class="order_filter" method="post" id="IdFilterData">
                                            ...
                                            <div class="row">
                                                ...
                                                @endif>
                                                    <label class="form-label" style="color:#0d0d0d ;">Client Wise</label>
                                                    <div class="list-inline text-center">
                                                        <div class="form-group">
                                ...
                                <ul class="nav nav-pills" id="myTab3" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link class_check active" id="profile-tab3" data-toggle="tab"
                                            ...
                                            onclick="exporthide(true)">Recharge History</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link class_check" id="contact-tab6" data-toggle="tab" href="#contact5"
                                            ...
                                            onclick="exporthide(true)">Shipment
                                            Transaction</a>
                                    </li>
                                    <li class="nav-item">
                                ...
                                <div class="tab-content excel" id="myTabContent2" style="text-align: center;">
                                    <div class="tab-pane fade show active my-2" id="profile3" role="tabpanel"
                                        ...
                                        <div class="table-responsive">
                                            <div class="totals">
                                                Total (This Page): {{currency_symbol()}} <span id="page_total">0.00</span> |
                                                Grand Total: {{currency_symbol()}} <span id="grand_total">0.00</span>
                                            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            var table1 = $('#profile3-table').DataTable({
                ...
                columns: [
                    { data: 'txn_date_time', orderable: false },
                    { data: 'client_name', orderable: false },
                    { data: 'txnid', orderable: false },
                    { data: 'banktxnid', orderable: false },
                    { data: 'amount', orderable: false },
                    { data: 'last_recharge', orderable: false },
                ...
                language: {
                    search: "_INPUT_",
                    searchPlaceholder: "Search By Reference ID & Bank Transaction."
                },
                dom: 'ftlip'
            });

            // Update totals after each AJAX draw
            table1.on('xhr.dt', function (e, settings, json) {
                $('#page_total').text(json.total_this_page ?? '0.00');
                $('#grand_total').text(json.grand_total ?? '0.00');
            });

            // Optional: trigger table reload on filter change
            $('#client, #date_range, #from_date, #to_date').on('change', function () {
                table1.ajax.reload();
            });
            ...
            var table = $('#failed_table').DataTable({
                ...
                language: {
                    search: "_INPUT_",
                    searchPlaceholder: "Search By AWB No & Reference ID ."
                },
                dom: 'ftlip',
                ajax: {
                    url: "{{ route('admin.shipment-transaction-list') }}",
                    type: "GET",
                    datatype: "json",
                    data: function (d) {
                        d.client = $('#client').val();
            ...
            var table3 = $('#db_table').DataTable({
                ...
                dom: 'ftlip',
                ajax: {
                    url: "{{ route('admin.db-transaction') }}",
                    type: "GET",
                    datatype: "json",
                    data: function (d) {
                        d.client = $('#client').val();
            ...
            var table2 = $('#cr_table').DataTable({
                ...
                dom: 'ftlip',
                ajax: {
                    url: "{{ route('admin.cr-transaction') }}",
                    type: "GET",
                    datatype: "json",
                    data: function (d) {
                        d.client = $('#client').val();
            ...
            $("#upload_report").click(function () {
                ...
                if (date_range) {
                    var exportUrl =
                        `{{ route('admin.export_wallet') }}?date_range=${date_range}&tableId=${tableId}&userId=${userId}`;
    ...
    <script>
        function showDetailsInModal(element) {
            // Show the modal
            var myModal = new bootstrap.Modal(document.getElementById('detailsModal'));

            // Get the data attributes from the clicked element
            const courierCharge = element.getAttribute('data-courier-charge') || 0;
            const taxAmount = element.getAttribute('data-tax-amount') || 0;
            const codCharge = element.getAttribute('data-cod-charge') || 0;
            const amount = element.getAttribute('data-amount') || 0;
            const sign = element.getAttribute('data-sign') || 0;

            // Fill the modal with the data
            document.getElementById('modalAmount').textContent = sign + amount;
            document.getElementById('modalCourierCharge').textContent = courierCharge;
            document.getElementById('modalTaxAmount').textContent = taxAmount;
            document.getElementById('modalCodCharge').textContent = codCharge;
            myModal.show();

            // Auto-hide the modal after 2 seconds (2000 milliseconds)
            setTimeout(() => {
                myModal.hide();
            }, 2000);
        }
