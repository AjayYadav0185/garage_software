...
<body>
    ...
    <div style="page-break-inside: avoid;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                ...
                <td style="text-align: left;">
                    <span style="display: inline-block; width: 100px;">{{ translate('Invoice No.:') }}</span> {{ $data->invoice_no }}
                </td>
            </tr>
            <tr>
                <td style="text-align: left;">
                    <span style="display: inline-block; width: 100px;">{{ translate('Invoice Period:') }}</span>
                    @php
                        $period = explode(' to ', $data->Invoice_Period);
                        if (count($period) == 2) {
                            try {
                                $start = \Carbon\Carbon::parse($period[0])->format('d-m-Y');
                                $end = \Carbon\Carbon::parse($period[1])->format('d-m-Y');
                                echo $start . ' To ' . $end;
                            } catch (Exception $e) {
                                echo $data->Invoice_Period; // fallback if parsing fails
                            }
                        } else {
                            echo $data->Invoice_Period; // if not in expected format
                        }
                    @endphp
                </td>
            ...
            <tr>
                ...
                <td style="vertical-align: top;">
                    <span style="display: inline-block; width: 100px;">{{ translate('Invoice Date:') }}</span>
                    {{ \Carbon\Carbon::parse($data->invoice_date)->format('d-m-Y') }}<br>
                    <div style="height: 1px; background-color: #000; margin: 4px 0;"></div>
                    <span style="display: inline-block; width: 100px;">{{ translate('Due Date:') }}</span>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 0px;">
            ...
            <tr>
                <td colspan="2" class="left">
                    ...
                    <span style="display: inline-block; width: 120px;">{{ translate('TAXIN/UIN:') }}</span> {{ $data->user_gst }}
                </td>
            </tr>
        </table>
        ...
        <table>
            ...
            <tr>
                <td class="center">{{ translate('1') }}</td>
                <td class="center">{{$data->service}}</td>
                <td class="center">{{ $data->hsn_sac }}</td>
                <td class="center">{{ $data->quantity }}</td>
                <td class="center">{{ nf($data->rate) }}</td>
                <td class="center"></td>
                <td class="right">{{ nf($data->amount) }}</td>
            </tr>
            ...
            <tr>
                ...
                <td class="right">{{ number_format($data->total ?? 0, 2) }}</td>
            </tr>
            {{-- <tr>
                <td colspan="5"></td>
        ...
        <table>
            ...
            <tr>
                <td class="center">{{ $data->hsn_sac }}</td>
                <td class="center">{{ nf($data->amount) }}</td>
                <td class="center">{{ nf($data->rate) }}%</td>
                <td class="center">{{ nf($data->amounts) }}</td>
                <td class="center">{{ nf($data->amounts) }}</td>
            </tr>
            <tr>
                ...
                <td></td>
                <td class="center"><b>{{ nf($data->amounts) }}</b></td>
                <td class="center"><b>{{ nf($data->amounts) }}</b></td>
            </tr>
        </table>
        ...
        <table style="border: 1px solid #000;">
            ...
            <tr>
                <td colspan="2" class="left">
                  ...
                  {{ translate('  Interest @ 18% p.a. will be charged if payment is not made within the stipulated time.') }}<br>
